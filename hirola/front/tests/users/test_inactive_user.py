"""Contains tests for user activation."""
...
class UserTest(BaseTestCase):
    """Tests activation state of user in the app."""
    def setUp(self):
        super(UserTest, self).setUp()

    def test_cron_inactive_user_delete(self):
        '''
        Test that if you have users with is_active set to False and that
        they have joined a period longer than
        settings.INACTIVE_EMAIL_EXPIRY_MINUTES_TIME.
        ...
        User.objects.create(
            ...
            last_name="Turimo", is_staff=False, is_active=False,
            change_email="turimo@gmail.com",
        ...
        User.objects.create(
            email="tripona@gmail.com", first_name="Tripona",
            last_name="Tirachi", is_staff=False, is_active=False,
            change_email="tirachi@gmail.com",
            change_email_tracker=timezone.now()-timedelta(minutes=minutes),
            date_joined=timezone.now()-timedelta(minutes=minutes),
            phone_number=72200000,
            )
        ...
        self.assertTrue(
            InactiveUser.objects.filter(first_name="Sivanna").first()
            )
        self.assertTrue(
            InactiveUser.objects.filter(first_name="Tripona").first()
            )
        ...
        self.assertEqual(
            inactive_sivanna.date_joined, sivanna_user.date_joined
            )
        self.assertEqual(
            inactive_sivanna.country_code, sivanna_user.country_code
            )
        self.assertEqual(
            inactive_sivanna.phone_number, sivanna_user.phone_number
            )
        self.assertEqual(inactive_sivanna.photo, sivanna_user.photo)
        self.assertEqual(
            inactive_sivanna.change_email, sivanna_user.change_email
            )
        self.assertEqual(
            inactive_sivanna.change_email_tracker,
            sivanna_user.change_email_tracker
            )
        self.assertEqual(
            inactive_sivanna.former_email, sivanna_user.former_email
            )
        ...
        self.assertEqual(
            inactive_triponna.first_name, triponna_user.first_name
            )
        self.assertEqual(
            inactive_triponna.last_name, triponna_user.last_name)
        self.assertEqual(
            inactive_triponna.date_joined,
            triponna_user.date_joined
            )
        self.assertEqual(
            inactive_triponna.country_code,
            triponna_user.country_code
            )
        self.assertEqual(
            inactive_triponna.phone_number,
            triponna_user.phone_number
            )
        self.assertEqual(
            inactive_triponna.photo, triponna_user.photo
            )
        self.assertEqual(
            inactive_triponna.change_email,
            triponna_user.change_email
            )
        self.assertEqual(
            inactive_triponna.change_email_tracker,
            triponna_user.change_email_tracker
            )
        self.assertEqual(
            inactive_triponna.former_email,
            triponna_user.former_email
            )
        self.assertEqual(
            inactive_triponna.password, triponna_user.password
            )

    def test_cron_inactive_user_non_delete(self):
        '''
        Test that if you have users with is_active set to True and that
        they have joined a period longer than
        settings.INACTIVE_EMAIL_EXPIRY_MINUTES_TIME.
        ...
        User.objects.create(
            email="tripona@gmail.com", first_name="Tripona",
            last_name="Tirachi", is_staff=False, is_active=True,
            change_email="tirachi@gmail.com",
            change_email_tracker=timezone.now()-timedelta(minutes=minutes),
            date_joined=timezone.now()-timedelta(minutes=minutes),
            phone_number=72200000,
